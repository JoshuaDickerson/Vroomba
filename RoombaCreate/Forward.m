function finalRad= Forward(serPort)
...
% CreateMatlabSim@gmail.com1

    % Set constants for this program
    maxDuration= 9999;  % Max time to allow the program to run (s)
    maxDistSansBump= 9999; % Max distance to travel without obstacles (m)
    ...
    maxOdomAng= pi/2;   % Max angle to move around a circle before 
    ...
    % Enter main loop
    while toc(tStart) < maxDuration && distSansBump <= maxDistSansBump %while the clock is running
...
%
% Input:
% serPort - Serial port object, used for communicating over bluetooth aka:
% the Roomba
%
% Output:
