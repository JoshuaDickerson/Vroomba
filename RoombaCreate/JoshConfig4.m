function finalRad= ControlProgram(serPort)
    ...
    stepCountX = 0;%Counts the number of "steps" the roomba has taken in the X coordinates 
    stepCountY = 0;%Counts the number of "steps" the roomba has taken in the Y coordinates
    turncountCW = 0; %Counts the number of clock-wise turns the Roomba has made
    turncountCCW = 0; %counts the number of counter clock-wise turns the Roomba has made
    ...
    while toc(tStart) < maxDuration && distSansBump <= maxDistSansBump
        ...
        %Preliminary start to a mapping program...needs a bit of work.
%         if DistanceSensorRoomba == 0 && angTurned == 0
%             stepCountX = stepCountX + DistanceSensorRoomba;
%         elseif DistanceSensorRoomba ~= 0 && angTurned ~= 0
%             stepCountY =stepCountY + DistanceSensorRoomba;
%             
%         end
