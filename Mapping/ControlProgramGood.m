...
function dispMap
rLog = load('roombaLog.dat'); %Sets up rLog as variable holding roombaLog.dat
distance = rLog(:,2); % The Radius is the second column of rLog (aka the distance)
distance2 = distance';%Transpose of the distance so that maths can be done with it
angle = rLog(:,3); % angle is the angle turned and the second column of rLog
theta = zeros();%Preallocating for efficiency the angle from degrees to radians
theta2 = zeros(); %Preallocating for the summation of all angles
for ii = 1:length(angle)%For-loop converts angles to radians
    theta(ii) = deg2rad(angle(ii)); %Converts angles to radians
    theta2 = sumtheta(theta(ii)); %Sums the current angle with all of the previous angles
    if theta(ii)> (2*pi) %Says if the converted angle is greater than 360 degrees
        theta(ii) = theta(ii)- (2*pi); %Subtract 360 degrees or 2pi radians from angle to get the angle in respect to the origin
    end
end
[x,y] = pol2cart(distance2,theta2);%Converts the polar coordinates to cartesian ones
plot(x,y) %plots the cartesian points
end
function rad = deg2rad(deg) % Takes in a degree input and spits out a radian output
rad = deg / 180 * pi;
end
